<template>
    <div class="flex items-center justify-between p-4">
        <RouterLink :to="{name: 'profile', params:{'id': post.created_by.id}}" class="flex items-center space-x-4">
            <div class="w-14 aspect-[1/1] rounded-full overflow-hidden">
                <img :src="post.created_by.get_avatar" class="w-full h-full object-cover">
            </div>
            
            <p>
                <strong>
                    <p class="text-lg">{{ post.created_by.name }}</p>
                </strong>
            </p>
        </RouterLink>
    ...
    >
        <div class="flex items-center justify-center">
            <img v-for="image in post.attachments" v-bind:key="image.id" :src="image.get_image" class="max-h-[700px] lg:max-w-[700px] md:max-w-full mb-4 rounded-xl rounded-lg">
        </div>
    </template>

    <p class="text-lg px-6">{{ post.body }}</p>

    <div class="my-6 px-6 flex justify-between">
